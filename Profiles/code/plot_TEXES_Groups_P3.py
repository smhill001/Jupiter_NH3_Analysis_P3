...
def plot_profile_L2(ax1,ax2,ax3,ax4,reference,ProfileHalfWidth=45.,LatPlotLims=[45,135],
    ...
    for obskey in DataSets[reference]:
        ...
        #######################################################################
        Lats,AvgProf,StdProf,CM2,amfAvgProf,patch,amfpatch=EP.extract_profile(pth,file+".fits",
        ...
        idx = (np.abs(Lats - (-50.))).argmin()
        ax3.annotate(file[5:15],xy=(-50,Avg_EW[idx]))
        idx = (np.abs(Lats - (50.))).argmin()
        ax3.annotate(file[5:15],xy=(50,Avg_EW[idx]))
        ax4.scatter(amfAvgProf,Avg_EW,s=1,label=file[5:15])
        ax4.scatter(amfAvgProf,Avg_EW,s=1,label=file[5:15])
